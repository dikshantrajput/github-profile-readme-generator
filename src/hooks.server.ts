...
const authGuard: Handle = async ({ event, resolve }) => {
  ...
  const authenticatedRoutesId = "/(authenticated)", unAuthenticatedRoutesId = "/(unauthenticated)";
  const isAuthenticatedRoute = event?.route?.id?.startsWith(authenticatedRoutesId)
  const isUnauthenticatedRoute = event?.route?.id?.startsWith(unAuthenticatedRoutesId)
  ...
  if (!session && (isAuthenticatedRoute || isAccessingRootRoute)) {
    ...
    throw redirect(303, `auth?redirect_to=${redirectToUrl !== "/" ? redirectToUrl : '/generate-readme'}`)
  }
...
export const handleError: HandleServerError = async ({ error }) => {
  ...
  return { id: errorId, message: (error as any)?.message ? (error as any)?.message : String(error) };
};
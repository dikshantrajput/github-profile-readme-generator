...
<div align="center">
  ...
  <h2>A Comprehensive Study of ${userProfile.name || userProfile.login}'s Development Portfolio</h2>
...
This paper presents a comprehensive analysis of the software development practices and contributions of ${userProfile.name || userProfile.login}. Through quantitative analysis of ${repos.length} repositories and ${totalStars + totalForks} total interactions, we demonstrate significant impact in the field of software development. ${userProfile.bio || ''}
...
<div align="center">
  ${repos
    ...
    .map((repo: any, index: number) => `
      <h4>Citation ${index + 1}: ${repo.name} [${repo.stargazers_count} citations]</h4>
...
${repos
  ...
  .slice(0, 5)
  .map((repo: any, index: number) => `
[${index + 1}] ${userProfile.login}, "${repo.name}," GitHub Repository, ${repo.created_at.split('T')[0]}.
...
<div align="center">
  <sub>This profile analysis was last updated on ${new Date().toLocaleDateString('en-US', { 
    year: 'numeric', 
    month: 'long', 
    day: 'numeric' 
  })}</sub>
